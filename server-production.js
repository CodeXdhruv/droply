...
// Droply Signaling Server - Production Ready
...
const log = {
  ...
  event: (event, socketId, data) => {
    if (NODE_ENV === 'development') {
      console.log(`📡 [${new Date().toISOString()}] EVENT: ${event} | Socket: ${socketId.substring(0, 8)}... | Data: ${JSON.stringify(data)}`);
    } else {
      console.log(`📡 [${new Date().toISOString()}] EVENT: ${event} | Socket: ${socketId.substring(0, 8)}...`);
    }
  }
};
...
app.get('/', (req, res) => {
  res.json({
    ...
    version: '1.0.0',
    status: 'running',
    message: 'WebRTC signaling server for P2P file transfers',
    docs: '/health'
  });
});
...
server.listen(PORT, () => {
  ...
  log.info(`🔍 Logs enabled for ${NODE_ENV} environment`);
...
const gracefulShutdown = (signal) => {
  ...
  server.close(() => {
    ...
    io.close();
    log.success('Socket.IO connections closed');
    
    // Exit after 5 seconds if processes still hanging
    setTimeout(() => {
